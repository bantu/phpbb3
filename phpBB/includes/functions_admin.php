...
*
* The heart of the function infact is call_user_func_array() PHP native
* function, which accepts data from array.
* Important:
...
* 1st array dimension:
*	key:	a string representing the name of the function to check with.
*			Example: 'ini_get', 'function_exists' and so on.
*			If there's phpbb wrapper function, it will be used instead of
*			native one, explicit definition of phpbb_ prefix is not needed.
*	value:	array of arrays, the latter contain actual data to check.
* 2nd dimension:
*	keys: 	no explicitly defined keys.
*	values:	arrays of data to be checked (see 3rd dimension description).
* 3rd dimension:
*	keys:
*		'class_callback':
*			to use callback function which belongs to some class. Example:
			'class_callback' => $auth
*		'check_value':
*			the string representing comma separated list of parameters, or
*			array of the same parameters, which will be passed to the function
*			defined as a 1st dimension key.
*			Example 1:	if 1st dimension key is 'version_compare' and 
*						'check_value' => array(PHP_VERSION, '5.2.0', '<')
*						is defined, version_compare(PHP_VERSION, '5.2.0', '<')
*						check will be performed.
*			Example 2:	if 1st dimension key is 'acl_get', and
*						'class_callback' => $auth, 'check_value' => 'a_server'
*						are defined, $auth->acl_get('a_server')
*						check will be performed.
*		'expected_value':
*			expected result of the check. If received, the check will be
*			considered as passed, no error row returned.
...
*			For checks where bitwise comparison is assumed and thus
*			'expected_value' is set to some integer/predefined constant, 
*			'comparison_type' => 'bitwise' should be set additionally.
*		'comparison_type'
*			should be set if bitwise comparison is assumed (the only case is
*			'comparison_type' => 'bitwise'). Can be skipped otherwise.
*		'negate':
*			should be set to true if the check result should not be equal to
*			'expected_value' value to pass the test. Can be skipped otherwise.
*		'check_extension_loaded':
*			additional check. If set to valid PHP extension name, main check
...
*		'ignore_errors':
*			if set, error control operator '@' should be used while performing
*			check, to supress errors output generated by the functions defined
*			as a 1st dimension key.
*		'additional_checks':
*			completely shadowing the main $checks_array structure to perform
*			additional recursive subchecks, which should be passed along with
*			main check row for overall check to be passed.
*
* @param bool	$additional_check	Boolean flag determining if subchecks
*									are to be performed (recursion)
*
...
*/
function phpbb_check_requirements($checks_array, $additional_check = false)
{
